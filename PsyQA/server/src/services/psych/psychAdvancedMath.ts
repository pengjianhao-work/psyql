...
export interface AdvancedMathSnapshot {
  /** Shannon 熵 H = -Σ p·log₂(p)，情绪分布无序度 */
  emotionEntropy: number;
  emotionEntropyNote: string;
  /** 压力序列 lag-1 自相关系数 ρ(1) */
...
export function mahalanobisDiagonal(
...
export function metricLevelText(level: string): string {
  return LEVEL_CN[level] ?? level;
}

export function computeAdvancedMath(
  stressSeries: number[],
  anxietySeries: number[],
  moodSeries: number[],
  emotionCounts: number[],
  currentStress: number,
  currentAnxiety: number
): AdvancedMathSnapshot {
  ...
  const prior: [number, number, number] = [48, 44, 58];
  ...
  const mahalanobisDistance = mahalanobisDiagonal(
    [currentStress, currentAnxiety, moodSeries[moodSeries.length - 1] ?? 58],
    prior,
    sigmas
  );
  ...
  if (entropyRatio >= 0.75) emotionEntropyNote = '情绪类型切换频繁，状态不稳定';
  else if (entropyRatio >= 0.45) emotionEntropyNote = '情绪分布中等离散';
