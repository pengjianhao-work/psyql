import {
  isLikelyThirdPersonCrisisMention,
  normalizeConfidence,
  scoreKeywordMatches
} from '../../utils/psychTextAnalysis';
...
export const PROBLEM_KEYWORDS: Record<ProblemCategory, string[]> = {
...
function scoreEmotionCategories(text: string): {
  scores: Record<EmotionType, number>;
  foundKeywords: Record<EmotionType, string[]>;
} {
  const scores = {} as Record<EmotionType, number>;
  const foundKeywords = {} as Record<EmotionType, string[]>;

  (Object.keys(EMOTION_KEYWORDS) as EmotionType[]).forEach((emotion) => {
    const { score, matched } = scoreKeywordMatches(text, EMOTION_KEYWORDS[emotion]);
...
export function analyzeEmotion(text: string, prior?: Pick<EmotionAnalysis, 'emotion' | 'confidence'>): EmotionAnalysis {
  ...
  const sortedEmotions = (Object.keys(scores) as EmotionType[])
    .filter((e) => scores[e] > 0)
  ...
  let confidence = normalizeConfidence(maxScore);
...
export function assessRisk(text: string): RiskAssessment {
  ...
  const applyLevel = (next: RiskLevel, msg: string) => {
    const order: RiskLevel[] = ['low', 'medium', 'high', 'critical'];
    if (order.indexOf(next) > order.indexOf(level)) {
      level = next;
      if (msg) warningMessage = msg;
    }
  };
  ...
  for (const [type, keywords] of Object.entries(CRISIS_KEYWORDS)) {
    ...
    for (const kw of sorted) {
      ...
      while ((idx = textLower.indexOf(kw, idx)) !== -1) {
        if (isLikelyThirdPersonCrisisMention(textLower, idx)) {
          idx += kw.length;
          continue;
        }
        ...
        if (type === 'suicide') {
          applyLevel(
            'critical',
            '⚠️ 检测到严重危机信号！生命宝贵，请立即联系专业心理咨询师或拨打心理援助热线！'
          );
...
export function analyzeProblem(text: string): ProblemAnalysis {
  const textLower = text.toLowerCase();
  const scores = {} as Record<ProblemCategory, number>;
  const foundKeywords = {} as Record<ProblemCategory, string[]>;

  (Object.keys(PROBLEM_KEYWORDS) as ProblemCategory[]).forEach((cat) => {
    const { score, matched } = scoreKeywordMatches(textLower, PROBLEM_KEYWORDS[cat]);
    scores[cat] = score;
    foundKeywords[cat] = matched;
  });

  let maxScore = 0;
  let detectedCategory: ProblemCategory = 'other';

  for (const category of Object.keys(scores) as ProblemCategory[]) {
  ...
  const allKeywords = (Object.keys(foundKeywords) as ProblemCategory[]).flatMap((c) => foundKeywords[c]);
  const confidence = normalizeConfidence(maxScore, 5);
...
function getSubcategories(category: ProblemCategory): string[] {
...
export function getEmotionStyle(emotion: EmotionType): {
  greeting: string;
  tone: string;
  color: string;
  emoji: string;
} {
  const styles: Record<EmotionType, { greeting: string; tone: string; color: string; emoji: string }> = {
    happy: {
...
export function getEmotionLabel(emotion: EmotionType | string): string {
  return EMOTION_LABELS[emotion as EmotionType] ?? String(emotion);
}

export function getRiskLabel(level: RiskLevel | string): string {
  return RISK_LABELS[level as RiskLevel] ?? String(level);
}

export function getCategoryName(category: ProblemCategory): string {
  const names: Record<ProblemCategory, string> = {
...
export function formatEmotionReport(
