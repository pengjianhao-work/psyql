...
import { findSimilarQuestions } from '../services/question/questionCatalog';
import { Question } from '../types';

describe('relevanceFilter', () => {
  test('hasDirectTopicOverlap detects shared phrases', () => {
    expect(hasDirectTopicOverlap('和舍友打架了', '宿舍矛盾怎么办')).toBe(true);
    expect(hasDirectTopicOverlap('和舍友打架了', '高三喜欢一个没有联系的人')).toBe(false);
  });
  ...
  test('filterSimilarQuestionsForDisplay drops weak matches', () => {
    ...
    const out = filterSimilarQuestionsForDisplay('今天和舍友打架了', undefined, items, 3);
  ...
  test('filterKnowledgeForDisplay keeps only related items', () => {
    ...
    const out = filterKnowledgeForDisplay('和舍友吵架', items, 3);
    expect(out.length).toBe(1);
...
describe('findSimilarQuestions', () => {
  ...
  test('returns empty for unrelated generic query context', () => {
    const result = findSimilarQuestions('今天天气真好', mockQuestions, 3);
    expect(result.length).toBe(0);
  });

  test('returns roommate-related for dorm conflict query', () => {
    const result = findSimilarQuestions('今天和舍友打架了', mockQuestions, 3);
    expect(result.length).toBeGreaterThan(0);
    expect(result.some((r) => r.question.includes('室友') || r.question.includes('宿舍'))).toBe(
      true
    );
