...
exports.runNeuralLayer = runNeuralLayer;
exports.runNeuralFromFeatures = runNeuralFromFeatures;
const psychFeatureEngine_1 = require("./psychFeatureEngine");
const INPUT_DIM = 19;
const HIDDEN_DIM = 16;
const W1 = Array.from({ length: HIDDEN_DIM }, (_, i) => Array.from({ length: INPUT_DIM }, (_, j) => {
    const seed = Math.sin(i * 12.9898 + j * 78.233) * 43758.5453;
    return (seed - Math.floor(seed) - 0.5) * 0.35;
}));
const B1 = Array.from({ length: HIDDEN_DIM }, () => 0.05);
...
function softmaxAttention(scores) {
...
function inferPrimaryConcern(f, attention) {
    var _a;
    const sorted = Object.entries(attention).sort((a, b) => b[1] - a[1]);
    const top = (_a = sorted[0]) === null || _a === void 0 ? void 0 : _a[0];
...
function runNeuralLayer(input, features) {
    ...
    const toScore = (weights) => clamp100(sigmoid(dot(hidden, weights)) * 100);
...
function runNeuralFromFeatures(features) {
    return runNeuralLayer((0, psychFeatureEngine_1.featuresToNeuralInput)(features), features);
}
