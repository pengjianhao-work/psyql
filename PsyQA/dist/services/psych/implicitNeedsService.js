...
exports.mineImplicitNeeds = mineImplicitNeeds;
...
function mineImplicitNeeds(userId, currentQuestion) {
    var _a;
    const history = (0, historyManager_1.getUserHistory)(userId);
    const texts = [
        currentQuestion,
        ...((history === null || history === void 0 ? void 0 : history.dialogs.slice(-8).map((d) => `${d.user} ${d.summary}`)) || [])
    ].join(' ');
    const hints = [];
    for (const p of IMPLICIT_PATTERNS) {
        if (!p.surface.test(texts))
            continue;
        const explicitAcademic = /考试|学业|成绩|考研/.test(currentQuestion);
        if (p.category === 'academic_stress' && explicitAcademic)
            continue;
        hints.push({
    ...
    const problemCounts = {};
    for (const d of (history === null || history === void 0 ? void 0 : history.dialogs.slice(-12)) || []) {
        if ((_a = d.psych) === null || _a === void 0 ? void 0 : _a.problem) {
