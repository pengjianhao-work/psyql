...
const reactCounselAgent_1 = require("../services/llm/reactCounselAgent");
describe('parseReActOutput', () => {
    test('parses thought action and json input', () => {
        const raw = `Thought: 需要先查知识库
Action: search_knowledge
Action Input: {"query":"宿舍矛盾"}`;
        const parsed = (0, reactCounselAgent_1.parseReActOutput)(raw);
    ...
    test('rejects unknown actions', () => {
        const parsed = (0, reactCounselAgent_1.parseReActOutput)('Action: fly_to_moon\nAction Input: {}');
        expect(parsed.action).toBeUndefined();
    });
    test('parse finish with answer', () => {
        const parsed = (0, reactCounselAgent_1.parseReActOutput)('Action: finish\nAction Input: {"answer":"这是一段足够长的测试回复，用于验证 finish 解析是否正常工作。"}');
...
describe('resolveReActPlan', () => {
    ...
    test('prefetch plan when knowledge prefetched', () => {
        const plan = (0, reactCounselAgent_1.resolveReActPlan)(Object.assign(Object.assign({}, baseCtx), { prefetchedKnowledge: [{ question: 'q', answer: 'a' }] }));
    ...
    test('demo mode disables prefetch shortcut', () => {
        ...
        if (prev === undefined)
            delete process.env.PSYQA_REACT_DEMO;
        else
            process.env.PSYQA_REACT_DEMO = prev;
    });
});
